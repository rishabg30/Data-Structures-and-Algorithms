...
vector<int> morris_preOrder_traversal(TreeNode *root)
{
    ...
    while (root != NULL)
    {
        ...
        else
        {
            ...
            // If rightMostNode->right is NULL, it means thread is not created [Have not traversed this path]
            ...
            //  If rightMostNode->right is not NULL, it means thread is created [Have traversed this path]
...
signed main()
{
    ...
    */
    ...
    node5->left = node8;
    node5->right = node9;
    vector<int> ans = morris_preOrder_traversal(root);
    printArray_1D(ans);
}