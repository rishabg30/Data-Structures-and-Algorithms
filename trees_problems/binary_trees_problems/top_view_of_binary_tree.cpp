...
vector<int> topView(TreeNode *root)
{
    ...
    while (q.size() > 0)
    {
        ...
        for (int i = 0; i < size_of_queue; i++)
        {
            ...
            // If we find node for the first time in a particular x_axis, that should be one of our answers
            if (mp.find(curr_node_x_axis) == mp.end())
            {
                mp.insert({curr_node_x_axis, curr_node});
            }
